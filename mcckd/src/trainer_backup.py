...
class DistributedTrainer_T5:
    ...
    def train_mcc(self,
        ...
        #indices1 = torch.cat([t.unsqueeze(0) for t in indices1], dim=0).to(logits1.device).T
        #indices2 = torch.cat([t.unsqueeze(0) for t in indices2], dim=0).to(logits2.device).T
        indices1 = torch.Tensor(indices1).to(logits1.device).to(torch.long)
        indices2 = torch.Tensor(indices2).to(logits2.device).to(torch.long)
        ...
        for i in range(bzs):
            ...
            # print(P.shape, Q.shape)   # both (1, 3, V=32128)
